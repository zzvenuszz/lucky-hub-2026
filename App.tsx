...
const App: React.FC = () => {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) { 
        alert('🎉 CHÚC MỪNG! Bạn đã đăng ký thành công.\n\nHãy đăng nhập ngay để bắt đầu theo dõi sức khỏe!'); 
        setLoginData({ username: regData.username, password: regData.password });
        setIsRegistering(false); 
      } else { 
        alert(result.message || 'Tên đăng nhập đã tồn tại.'); 
      }
      ...
      <div className="min-h-screen bg-emerald-600 flex items-center justify-center p-4">
        <div className="bg-white p-8 rounded-[2.5rem] shadow-2xl w-full max-w-md animate-in zoom-in-95">
          <div className="text-center mb-6">
            ...
            <p className="text-slate-400 text-sm mt-1">Health Management System</p>
          </div>
          ...
          {needsUpgrade ? (
            ...
            <form className="space-y-4" onSubmit={handleLogin}>
              <div className="space-y-1">
                ...
                <input required placeholder="Username..." inputMode="text" autoCapitalize="none" autoCorrect="off" spellCheck="false" value={loginData.username} onChange={e => setLoginData({...loginData, username: e.target.value})} className="w-full px-4 py-3 rounded-xl bg-slate-50 border border-transparent focus:border-emerald-500 outline-none transition-all font-medium" />
              </div>
              <div className="space-y-1">
                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Mật khẩu</label>
                <div className="relative">
                  <input required type={showLoginPass ? "text" : "password"} placeholder="Mật khẩu..." value={loginData.password} onChange={e => setLoginData({...loginData, password: e.target.value})} className="w-full px-4 py-3 rounded-xl bg-slate-50 border border-transparent focus:border-emerald-500 outline-none transition-all font-medium pr-12" />
            ...
            <form className="space-y-3 max-h-[70vh] overflow-y-auto px-1 no-scrollbar" onSubmit={handleRegister}>
              <div className="grid grid-cols-1 gap-3">
                <div className="space-y-1">
                  ...
                  <input placeholder="Tên của bạn..." required value={regData.fullName} onChange={e => setRegData({...regData, fullName: e.target.value})} className="w-full px-4 py-2.5 rounded-xl bg-slate-50 border outline-none focus:border-emerald-500" />
                </div>
                <div className="space-y-1">
                  <label className="text-[10px] font-black text-slate-400 uppercase ml-1">Tên đăng nhập (Username)</label>
                  <input placeholder="Viết liền không dấu..." inputMode="text" autoCapitalize="none" autoCorrect="off" spellCheck="false" required value={regData.username} onChange={e => setRegData({...regData, username: e.target.value})} className="w-full px-4 py-2.5 rounded-xl bg-slate-50 border outline-none focus:border-emerald-500" />
                </div>
                <div className="space-y-1">
                  <label className="text-[10px] font-black text-slate-400 uppercase ml-1">Mật khẩu</label>
                  <div className="relative">
                    <input placeholder="Mật khẩu của bạn..." type={showRegPass ? "text" : "password"} required value={regData.password} onChange={e => setRegData({...regData, password: e.target.value})} className="w-full px-4 py-2.5 rounded-xl bg-slate-50 border outline-none focus:border-emerald-500 pr-12" />
