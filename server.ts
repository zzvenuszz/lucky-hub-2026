...
async function callAIWithRetry(requestId: string, modelName: string, payload: any, retries = 3): Promise<any> {
  ...
  while (attempt < retries) {
    ...
    if (availableKeys.length === 0) throw new Error("Hệ thống AI đang bảo trì (Cooldown). Thử lại sau 30 giây.");
...
app.post('/api/ai/bulk-extract', async (req, res) => {
  ...
  try {
    const { imageBase64 } = req.body;
    const payload = {
      contents: [{ parts: [{ text: "Extract list of metrics from notebook to JSON array." }, { inlineData: { data: imageBase64, mimeType: 'image/jpeg' } }] }],
      config: {
        responseMimeType: "application/json",
        responseSchema: {
          type: Type.ARRAY,
          items: {
            type: Type.OBJECT,
            properties: { weight: { type: Type.NUMBER }, bodyFat: { type: Type.NUMBER }, muscleMass: { type: Type.NUMBER }, date: { type: Type.STRING } }
          }
        }
      }
    };
...
app.listen(PORT, () => console.log(`🚀 Lucky Hub 2026 is LIVE on port ${PORT}`));
