...
export const extractMetricsFromImage = async (base64Image: string): Promise<Partial<HealthMetric> | null> => {
  ...
  try {
    ...
    if (!res.ok) throw new Error(`Lỗi Server AI: ${res.status}`);
    
    const result = await res.json();
    if (!result.weight || result.weight <= 0) return null;
...
Chỉ số mới nhất: ${latestMetric ? `Cân nặng ${latestMetric.weight}kg, Mỡ cơ thể ${latestMetric.bodyFat}%` : "Chưa có dữ liệu"}.
QUY TẮC: ${systemRules}
KIẾN THỨC: ${contextKnowledge}
PHONG CÁCH: Chuyên nghiệp, tận tâm, trả lời ngắn gọn.`;

    // LUÔN GIỚI HẠN 10 TIN NHẮN ĐỂ TIẾT KIỆM TOKEN
