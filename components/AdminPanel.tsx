    ...
    <div className="bg-white rounded-[2.5rem] shadow-sm border border-slate-100 overflow-hidden min-h-[75vh] flex flex-col">
      <div className="flex flex-col md:flex-row bg-slate-50/50 p-4 m-6 rounded-3xl border border-slate-100 gap-4 shrink-0">
        ...
        <div className="flex items-center gap-2 bg-white/50 p-1 rounded-2xl px-4">
          <button onClick={() => setShowConsole(!showConsole)} className={`w-8 h-8 rounded-lg flex items-center justify-center transition-all ${showConsole ? 'bg-emerald-600 text-white shadow-lg' : 'bg-slate-100 text-slate-400'}`}>📟</button>
        </div>
      </div>
      ...
      <div className="px-8 pb-8 flex-grow overflow-y-auto no-scrollbar">
        ...
        {activeTab === 'audit' && (
          <div className="space-y-6 animate-in fade-in">
            <div className="flex items-center gap-4 bg-slate-50 p-4 rounded-2xl">
              <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Lọc sự kiện:</span>
              <select 
                value={auditFilter} 
            ...
            <div className="overflow-x-auto border border-slate-50 rounded-2xl">
              <table className="w-full text-[11px] text-left">
                ...
                <tbody className="divide-y divide-slate-50">
                      ...
                      <td className="p-4">
                        <span className={`px-2 py-1 rounded-lg text-[8px] font-black uppercase ${
                          ...
                          {log.type === AuditLogType.REGISTER ? 'Đăng ký' :
                           ...
                           log.type === AuditLogType.METRIC_HELP_UPDATE ? 'Cập nhật hộ' : 'Tự cập nhật'}
                        </span>
                      </td>
                      <td className="p-4">
                        {log.targetName ? (
                  ...
                  {auditLogs.length === 0 && (
                    <tr><td colSpan={5} className="p-10 text-center text-slate-300 italic">Chưa có dữ liệu nhật ký</td></tr>
                  )}
                </tbody>
              </table>
            </div>
        ...
        {/* Các tab khác giữ nguyên logic hiện tại */}
        {activeTab === 'metrics' && (
                      ...
                      {userMetrics.map(m => (
                        <tr key={m.id || (m as any)._id} className="hover:bg-slate-50/50">
                          ...
                          <td className="p-3 text-right">
                             <button onClick={() => setEditingMetric(m)} className="text-emerald-600 font-black text-[9px] uppercase hover:underline">Sửa</button>
                          </td>
                        </tr>
                      ))}
                    </tbody>
