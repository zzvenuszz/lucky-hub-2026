...
import PostCreator from './newsfeed/PostCreator.tsx';
import PostItem from './newsfeed/PostItem.tsx';
...
const NewsFeed: React.FC<NewsFeedProps> = ({ currentUser }) => {
  ...
  const handleCreatePost = async () => {
    ...
    if (saved) { setPosts([saved, ...posts]); setInputText(''); setSelectedImages([]); }
  ...
  const handleReaction = async (postId: string, type: string) => {
    ...
    if (updatedPost) setPosts(prev => prev.map(p => (p.id || (p as any)._id) === (updatedPost.id || (updatedPost as any)._id) ? { ...updatedPost, id: updatedPost.id || (updatedPost as any)._id } : p));
