...
const SystemLog: React.FC<SystemLogProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <div className="fixed bottom-[155px] right-6 w-[400px] max-w-[90vw] h-[450px] max-h-[60vh] bg-slate-900/95 backdrop-blur-xl border border-slate-700 rounded-[2.5rem] shadow-2xl flex flex-col overflow-hidden z-[1001] animate-in slide-in-from-bottom-4 duration-300">
      ...
      <div className="p-5 border-b border-slate-800 shrink-0">
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-center gap-3">
            <button onClick={clearLogs} className="text-[9px] font-black text-rose-400 uppercase hover:text-rose-300 transition-colors">Clear</button>
            <button onClick={onClose} className="text-[12px] text-slate-500 hover:text-white transition-colors">✕</button>
          </div>
        </div>
      ...
      <div className="p-3 bg-slate-950 border-t border-slate-800 text-center">
         <p className="text-[8px] font-bold text-slate-500 uppercase tracking-widest italic">(*) Lucky Hub Monitoring System</p>
      </div>
    </div>
  );
};
