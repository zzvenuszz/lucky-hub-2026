...
const ChatSystem: React.FC<ChatSystemProps> = ({ currentUser, users, knowledge, rules, onClose }) => {
  ...
  const handleSendMessage = async () => {
    ...
    if (!isTargetAI) {
      ...
      if (matchedKnowledge) {
        updatedMessages.push({ id: `msg_prompt_${Date.now()}`, senderId: 'ai_coach', senderName: '🍀Trợ lý Lucky', senderRole: 'AI' as any, content: AI_PROMPT_TEXT, timestamp: new Date().toISOString() });
      }
    }
    ...
    if (isTargetAI) {
      setIsTypingAI(true);
      try {
        const aiResponse = await getAICoachResponse(updatedChat.messages, knowledge, rules, sentText || "Phân tích hình ảnh này cho tôi", currentUser.healthGoal, latestMetric, base64Data);
        if (aiResponse) {
        ...
        {showContacts ? (
          ...
          <>
              ...
              {isTypingAI && (
                <div className="flex flex-col items-start px-2">
                  <div className="bg-amber-50 border border-amber-100 text-amber-600 p-2 rounded-xl rounded-tl-none flex items-center gap-2">
                    ...
                    <span className="text-[9px] font-black uppercase tracking-widest ml-1">Lucky đang soạn phản hồi...</span>
                  </div>
                </div>
              )}
            </div>
            ...
            <div className="p-4 bg-white border-t border-slate-50 shrink-0">
              {isCompressing && <p className="text-[9px] text-emerald-600 font-bold mb-1 animate-pulse">⏳ Lucky đang tối ưu ảnh...</p>}
              {selectedImage && (
